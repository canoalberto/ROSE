...
import moa.evaluation.WindowImbalancedClassificationPerformanceEvaluator;
...
public class ROSE extends AbstractClassifier implements MultiClassClassifier, CapabilitiesHandler {
	...
	protected WindowImbalancedClassificationPerformanceEvaluator evaluator;
	...
	public void resetLearningImpl() {
		...
		this.evaluator = new WindowImbalancedClassificationPerformanceEvaluator();
	}
			...
			if(this.warningDetected) {
				...
				for(int i = 0; i < this.ensembleSizeOption.getValue(); i++) {
					...
					this.ensembleBackground[i] = new ROSEBaseLearner(
							...
							(WindowImbalancedClassificationPerformanceEvaluator) classificationEvaluator.copy(), 
							this.instancesSeen, 
	...
	public String getPurposeString() {
		return "Online Self Adjusting Kappa Updated Ensemble";
	}
	...
	protected final class ROSEBaseLearner extends AbstractMOAObject {
		...
		// Statistics
		public WindowImbalancedClassificationPerformanceEvaluator evaluator;
		protected int numberOfDriftsDetected;
		protected int numberOfWarningsDetected;

		public ROSEBaseLearner (RandomSubspaceHT instantiatedClassifier, WindowImbalancedClassificationPerformanceEvaluator evaluatorInstantiated, long instancesSeen, ClassOption driftOption, ClassOption warningOption, boolean isBackgroundLearner) {
		...
		public void trainOnInstance(Instance instance, double weight, long instancesSeen) {
			...
			if(!this.isBackgroundLearner) {
				...
				if(this.warningDetectionMethod.getChange()) {
					...
					WindowImbalancedClassificationPerformanceEvaluator bkgEvaluator = (WindowImbalancedClassificationPerformanceEvaluator) this.evaluator.copy();
					bkgEvaluator.reset();
