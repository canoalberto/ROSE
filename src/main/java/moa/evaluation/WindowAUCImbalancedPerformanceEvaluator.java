...
public class WindowAUCImbalancedPerformanceEvaluator extends
	...
	public class Estimator {
		...
		public void add(double score, boolean isPositive, boolean correctPrediction) {
			...
			if (size > 0 && posWindow >= this.size) {
				...
				this.confusionMatrix[oldestExamplePredictedClass][oldestExampleTrueClass]--;
			}
	...
	protected double AUC(int[][] confusionMatrix, int positiveClass, int negativeClass)
	{
		int tp = confusionMatrix[positiveClass][positiveClass];
		int fp = confusionMatrix[positiveClass][negativeClass];
		int tn = confusionMatrix[negativeClass][negativeClass];
		int fn = confusionMatrix[negativeClass][positiveClass];
	...
	public Measurement[] getPerformanceMeasurements() {
		...
		Measurement[] measurement = new Measurement[11 + numClasses*numClasses];

		measurement[0] = new Measurement("classified instances", this.totalObservedInstances);
		measurement[1] = new Measurement("AUC", AUC(this.aucEstimator.confusionMatrix, 1, 0));
		measurement[1] = new Measurement("pAUC", this.aucEstimator.getAUC());
		measurement[2] = new Measurement("sAUC", this.aucEstimator.getScoredAUC());
		measurement[3] = new Measurement("Accuracy", this.aucEstimator.getAccuracy());
		measurement[4] = new Measurement("Kappa", this.aucEstimator.getKappa());
		measurement[5] = new Measurement("Periodical holdout AUC", this.aucEstimator.getHoldoutAUC());
		measurement[6] = new Measurement("Pos/Neg ratio", this.aucEstimator.getRatio());
		...
		for(int i = 0; i < numClasses; i++) {
			for(int j = 0; j < numClasses; j++) {
				measurement[11 + i*numClasses + j] = new Measurement("CM["+i+"]["+j+"]", this.aucEstimator.confusionMatrix[i][j]);
			}
		}
