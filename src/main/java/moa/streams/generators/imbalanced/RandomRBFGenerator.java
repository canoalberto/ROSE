...
InstanceStream {
	...
	public FloatOption imbalanceRatio = new FloatOption("imbalanceRatio", 'm',
			"Percentage of minority class examples", 0.1, 0, 1);
	...
	public InstanceExample nextInstance() {
		...
		int label = instanceRandom.nextDouble() < imbalanceRatio.getValue() ? 1 : 0;

		do{
			centroid = this.centroids[MiscUtils.chooseRandomIndexBasedOnWeights(this.centroidWeights, this.instanceRandom)];
		}while(centroid.classLabel != label);
